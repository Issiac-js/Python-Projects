...
def center_window(self, w, h): #pass  in the tkinter frame (master) reference and the w and h
    ...
    #calculate x and y coordinates to point the app centered on the user's screen
...
def create_db(self):
    conn = s3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
...
def addToList(self):
    ...
    var_fname = var_fname.strip() #this removes any spaces before or after the entry
    var_lname = var_lname.strip()

    var_fname = var_fname.title() #this ensures the first letter is capitalized.
    ...
    if (len(var_fname)> 0) and (len(var_lname)> 0) and (len(var_phone) > 0) and (len(var_email) >0): #enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName ==0: #if this is 0 then there is no existance of the fullname and we add new data
...
def onDelete(self):
    ...
    with conn:
        ...
        #the database... cannot delete last record in or we will get an error.
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    ...
                    # onRefresh(self) #update the listbox changes -- sounds like a good idea
                    conn.commit()
        else:
